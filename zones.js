...
export class ZoneManager {
  ...
  _drawPoly(zone, isAlert) {
    ...
    const lw=ctx.measureText(zone.name).width;
    ctx.fillStyle=sc; ctx.fillRect(cx*cw-lw/2-5, cy*ch-18, lw+10, 16);
    ctx.fillStyle='#000'; ctx.fillText(zone.name, cx*cw-lw/2, cy*ch-5);
    ctx.restore();
  }
  ...
  /* ── Firebase ────────────────────────────────────────── */
  toFirebaseData() {
    return this.zones.map(z=>({id:z.id,name:z.name,points:z.points,colorIdx:z.colorIdx}));
  }
  loadFromFirebase(data) {
    this.zones=data.map(z=>({...z,alert:false}));
    if (this._onZoneChange) this._onZoneChange(this.zones);
  }
}